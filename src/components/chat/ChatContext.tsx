...
import { ChatMessage, ConversationId, ConversationSummary, getConversationId } from "./types";
...
type ChatContextValue = {
  ...
  unreadMap: Record<ConversationId, number>;
  clearUnread: (conversationId: ConversationId) => void;
  clearAllUnread: () => void;
...
export function ChatProvider({ children }: { children: React.ReactNode }) {
  ...
  const [unreadMap, setUnreadMap] = useState<Record<ConversationId, number>>({});
  const [typingMap, setTypingMap] = useState<Record<ConversationId, boolean>>({});
  ...
  const clearUnread = useCallback((conversationId: ConversationId) => {
    setUnreadMap((m) => ({ ...m, [conversationId]: 0 }));
    if (currentUserId && openWithUserId) {
  ...
  // Compute conversation summaries from storage on demand (lightweight: using localStorage snapshot)
  const conversations = useMemo<ConversationSummary[]>(() => {
    if (!currentUserId) return [];
    try {
      const raw = localStorage.getItem("skillSync.chat.messages");
      const all = raw ? (JSON.parse(raw) as Record<ConversationId, ChatMessage[]>) : {};
      const items: ConversationSummary[] = Object.entries(all)
  ...
  useEffect(() => {
    ...
    if (totalUnread > 0) {
      ...
      // restore default if possible by removing data URL so browser reloads original
      const original = document.querySelector<HTMLLinkElement>('link[rel="icon"][data-original="true"]') as HTMLLinkElement;
      if (original) {
...
if (typeof window !== "undefined") {
  ...
  const updateTitle = () => {
    try {
      const raw = localStorage.getItem("skillSync.chat.messages");
      // We cannot compute unread from storage alone; rely on a lightweight event.
      // Instead, listen to our custom event and recompute from unreadMap via a hidden channel is overkill.
      // Simple approach: we update when message events fire.
    } catch {}
  };
  // Listen to our custom broadcast to bump a simple counter in the title if page hidden
  window.addEventListener("skillSyncChat", (e: any) => {
    const ev = e.detail;
